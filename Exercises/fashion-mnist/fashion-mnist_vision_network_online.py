...
with nengo.Network(seed=3) as model:
    ...
    confidence_node = nengo.Ensemble(1, neuron_type=nengo.Direct(), dimensions=n_out, label="Confidence")
    ...
    config[nengo.Ensemble].neuron_type = nengo.Direct()
    with config:
        output_spa_true = nengo.spa.State(len(vocab_names), subdimensions=n_out, vocab=vocab)
    
    nengo.Connection(true_output, output_spa_true.input)
    
print(output_spa.output)
print("Done building")